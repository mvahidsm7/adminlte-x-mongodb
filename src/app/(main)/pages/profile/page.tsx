...
const activity = [
  {
    ...
    target: "AdminLTE Next.js",
    time: "2 hours ago",
    color: "bg-mongo-green",
  },
  {
    action: "Closed issue",
    target: "Fix sidebar bug",
    time: "5 hours ago",
      ...
      <Card>
        <CardBody>
          ...
          <p className="text-sm text-ink leading-relaxed mb-4">
            Full-stack developer specializing in React and Node.js. Passionate
            about building clean, maintainable code and great user experiences.
          </p>
          <ul className="space-y-2 text-sm text-steel">
            ...
            <li className="flex items-center gap-2">
              <Globe className="w-4 h-4" /> adminlte.io
            </li>
            <li className="flex items-center gap-2">
              <GitBranch className="w-4 h-4" /> github.com/admin
            </li>
            <li className="flex items-center gap-2">
              <AtSign className="w-4 h-4" /> @adminlte
            </li>
          </ul>
        </CardBody>
      </Card>
        ...
        <ol className="relative border-l border-hairline ml-3 space-y-6">
          {activity.map((item, i) => (
            <li key={i} className="ml-6">
              <span
                className={`absolute -left-1.5 w-3 h-3 rounded-full ${item.color}`}
              />
    ...
    <Card>
      <CardBody>
        ...
        <p className="text-sm text-steel">
          Visit <span className="font-medium text-ink">Settings</span> for full
          account preferences.
        </p>
      </CardBody>
    </Card>
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardBody className="relative pt-0">
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 -mt-4 mb-4">
            <Avatar
              name="Admin User"
              size="xl"
              className="border-4 border-white shadow-level-2 shrink-0"
            />
            <div className="flex-1 pb-1">
              <h1 className="text-xl font-bold text-ink">Admin User</h1>
              <p className="text-sm text-steel">
                Software Engineer · AdminLTE Inc.
              </p>
            </div>
